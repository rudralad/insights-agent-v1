...
def submit_rating():
    ...
    if st.session_state.research_history:
        ...
        try:
            ...
            print(f"Warning: Invalid rating value: {st.session_state.current_rating}")
            rating_value = 3  # Default to 3 if invalid
        ...
        print(f"Rating: {rating_value}, Feedback: {st.session_state.rating_feedback}")
        ...
        if (
            ...
            for analytics in st.session_state.analytics_history:
                if analytics.get("research_id") == last_entry.get("id"):
                    ...
                    analytics["result_metrics"]["rating_timestamp"] = rating_timestamp
...
# Add search data debug section (hidden by default)
if st.session_state.all_search_history:
    with st.sidebar.expander("Debug: Search Data (for DB)", expanded=False):
        st.write("Latest search data stored for database:")
        latest_search = st.session_state.all_search_history[-1]

        # Show search queries
        st.write(f"**Query:** {latest_search['query']}")
        st.write(f"**User ID:** {latest_search['user_id']}")
        st.write(f"**Timestamp:** {latest_search['timestamp']}")
        st.write(
            f"**Number of search queries:** {len(latest_search['search_queries'])}"
        )
        st.write(f"**Number of unique URLs:** {len(latest_search['unique_urls'])}")
        st.write(f"**Total results:** {latest_search['num_results']}")

        # Show search queries without using an expander
        st.markdown("#### Search Queries")
        st.markdown(
            "<details><summary>Click to expand</summary>", unsafe_allow_html=True
        )
        for i, query in enumerate(latest_search["search_queries"]):
            st.write(f"{i+1}. {query}")
        st.markdown("</details>", unsafe_allow_html=True)

        # Show a sample of the results without using an expander
        st.markdown("#### Sample Results (first 5)")
        st.markdown(
            "<details><summary>Click to expand</summary>", unsafe_allow_html=True
        )
        for i, result in enumerate(latest_search["detailed_results"][:5]):
            st.markdown(f"**{i+1}. [{result['title']}]({result['url']})**")
            st.write(f"From query: '{result['search_query']}'")
            st.markdown("---")
        st.markdown("</details>", unsafe_allow_html=True)

        # Show scraped content information without using an expander
        if "scraped_sources" in latest_search and latest_search["scraped_sources"]:
            st.markdown("#### Scraped Content Info")
            st.markdown(
                "<details><summary>Click to expand</summary>", unsafe_allow_html=True
            )
            st.write(
                f"**Total sources scraped:** {len(latest_search['scraped_sources'])}"
            )

            for i, source in enumerate(
                latest_search["scraped_sources"][:3]
            ):  # Show first 3 only
                st.markdown(f"**Source {i+1}: {source['url']}**")
                st.write(f"Content length: {source['content_length']} characters")
                st.text_area(
                    f"Preview {i+1}",
            ...
            if len(latest_search["scraped_sources"]) > 3:
                st.write(
                    f"*...and {len(latest_search['scraped_sources']) - 3} more sources*"
                )
            st.markdown("</details>", unsafe_allow_html=True)

# Add analytics debug section
if "analytics_history" in st.session_state and st.session_state.analytics_history:
    with st.sidebar.expander("Debug: Research Analytics (for DB)", expanded=False):
        st.write("Latest research analytics stored for database:")
        latest_analytics = st.session_state.analytics_history[-1]

        # Section 1: Research identification
        st.markdown("### Research Information")
        st.write(f"**Research ID:** {latest_analytics['research_id']}")
        st.write(f"**Query:** {latest_analytics['query_data']['original_query']}")

        # Section 2: Timing information
        st.markdown("### Timing Information")
        st.write(f"**Started:** {latest_analytics['query_data']['timestamp_started']}")
        st.write(
            f"**Completed:** {latest_analytics['query_data']['timestamp_completed']}"
        )
        st.write(
            f"**Total time:** {latest_analytics['query_data']['total_time_seconds']:.2f} seconds"
        )

        # Section 3: Phase timing metrics
        st.markdown("### Phase Timing")
        st.markdown(
            "<details><summary>Click to expand</summary>", unsafe_allow_html=True
        )
        timing_data = latest_analytics["processing_metrics"]
        ...
        times = [
            timing_data["query_generation_time"],
            timing_data["search_time"],
            timing_data["scraping_time"],
            timing_data["insight_extraction_time"],
            timing_data["report_generation_time"],
        ]

        # Display times in a table
        st.write("Time spent in each phase (seconds):")
        for phase, time in zip(phases, times):
            st.write(f"**{phase}:** {time:.2f}s")
        ...
        if total > 0:
            ...
            for phase, time in zip(phases, times):
                ...
                st.write(f"**{phase}:** {percentage:.1f}%")
        st.markdown("</details>", unsafe_allow_html=True)

        # Section 4: Search metrics
        st.markdown("### Search Metrics")
        st.markdown(
            "<details><summary>Click to expand</summary>", unsafe_allow_html=True
        )
        search_metrics = latest_analytics["search_metrics"]
        st.write(
            f"**Search queries generated:** {search_metrics['num_search_queries']}"
        )
        st.write(f"**Total results found:** {search_metrics['total_results_found']}")
        st.write(f"**Unique URLs discovered:** {search_metrics['unique_urls_found']}")
        st.markdown("</details>", unsafe_allow_html=True)

        # Section 5: Scraping metrics
        st.markdown("### Scraping Metrics")
        st.markdown(
            "<details><summary>Click to expand</summary>", unsafe_allow_html=True
        )
        scrape_metrics = latest_analytics["scraping_metrics"]
        st.write(f"**URLs attempted:** {scrape_metrics['total_urls_attempted']}")
        st.write(f"**Successful scrapes:** {scrape_metrics['successful_scrapes']}")
        st.write(f"**Failed scrapes:** {scrape_metrics['failed_scrapes']}")

        success_rate = 0
        if scrape_metrics["total_urls_attempted"] > 0:
            success_rate = (
                scrape_metrics["successful_scrapes"]
                / scrape_metrics["total_urls_attempted"]
            ) * 100
        st.write(f"**Scraping success rate:** {success_rate:.1f}%")

        st.write(
            f"**Total content extracted:** {scrape_metrics['total_content_chars']:,} characters"
        )
        st.write(
            f"**Average content length:** {scrape_metrics['average_content_length']:.1f} characters"
        )
        st.markdown("</details>", unsafe_allow_html=True)

        # Section 6: Result metrics
        st.markdown("### Result Metrics")
        st.markdown(
            "<details><summary>Click to expand</summary>", unsafe_allow_html=True
        )
        result_metrics = latest_analytics["result_metrics"]
        st.write(
            f"**Report content length:** {result_metrics['report_content_length']:,} characters"
        )
        st.write(f"**Number of citations:** {result_metrics['num_citations']}")

        # Rating info if available
        if result_metrics["rating"]:
            st.write(f"**User rating:** {result_metrics['rating']}/5 stars")
            if result_metrics["rating_feedback"]:
                st.write(f"**Feedback:** {result_metrics['rating_feedback']}")
        else:
        ...
        user_data = latest_analytics["user_data"]
        st.write(f"**User ID:** {user_data['user_id']}")
        st.write(f"**Name:** {user_data['user_name']}")
        st.write(f"**Email:** {user_data['user_email']}")
        st.write(f"**Registered:** {user_data['user_registered']}")
        st.markdown("</details>", unsafe_allow_html=True)
