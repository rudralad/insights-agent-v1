...
BASE_ID = None  # Will be loaded from config
USERS_TABLE_ID = None  # Will be loaded from config
RESEARCH_TABLE_ID = None  # Will be loaded from config
SEARCH_DATA_TABLE_ID = None  # Will be loaded from config
ANALYTICS_TABLE_ID = None  # Will be loaded from config
...
def save_config(config: Dict[str, Any]):
    """
    Save the current Airtable configuration to a file.
    """
    with open(CONFIG_FILE, "w") as f:
        json.dump(config, f, indent=4)

    print(f"Airtable configuration saved to {CONFIG_FILE}")


def load_config():
    """
    Load Airtable configuration from a file.
    ...
    try:
        with open(CONFIG_FILE, "r") as f:
            config = json.load(f)
            print(f"Read configuration: {json.dumps(config, indent=2)}")

        BASE_ID = config.get("base_id")

        # Verify base ID format
        if not BASE_ID:
            print("No base ID found in config")
            return False
        ...
        USERS_TABLE_ID = config.get("users_table_id")
        RESEARCH_TABLE_ID = config.get("research_table_id")
        SEARCH_DATA_TABLE_ID = config.get("search_data_table_id")
        ANALYTICS_TABLE_ID = config.get("analytics_table_id")

        # Check if all table IDs are present
        missing_tables = []
        if not USERS_TABLE_ID:
            missing_tables.append("users_table_id")
        if not RESEARCH_TABLE_ID:
            missing_tables.append("research_table_id")
        if not SEARCH_DATA_TABLE_ID:
            missing_tables.append("search_data_table_id")
        if not ANALYTICS_TABLE_ID:
            missing_tables.append("analytics_table_id")

        if missing_tables:
            print(f"Missing table IDs in config: {', '.join(missing_tables)}")
            return False
        ...
        # Instead of using the meta API, try to access one of the tables directly
        # This is more likely to work with personal access tokens
        url = f"{AIRTABLE_API_URL}/{BASE_ID}/{USERS_TABLE_ID}"
        print(f"Verifying base access with URL: {url}")

        headers = get_headers()
        print(
            f"Using headers with API key: {'*' * (len(AIRTABLE_API_KEY) - 4) + AIRTABLE_API_KEY[-4:]}"
        )
        ...
        print(f"Airtable configuration loaded from {CONFIG_FILE}")
        print(f"Using existing base: {BASE_ID}")
        return True
    except Exception as e:
        print(f"Error loading Airtable configuration: {str(e)}")
        import traceback
...
def create_research_session(research_data: Dict[str, Any]) -> str:
    ...
    print(f"Research session payload: {payload}")
...
def update_research_session(research_id: str, update_data: Dict[str, Any]) -> None:
    ...
    if not data.get("records"):
        ...
        if not data.get("records"):
            # List all research sessions to debug
...
def initialize_airtable() -> bool:
    ...
    try:
        ...
        result = load_config()
        if result:
            print("Successfully loaded Airtable configuration")
            return True
        else:
            print("Failed to load valid Airtable configuration")

        # No valid configuration found, provide instructions to manually set up
        print("\n" + "=" * 80)
        ...
        print("3. Copy the base ID from the URL (it looks like 'appXXXXXXXXXXXXXX')")
        print(
            "4. Copy the table IDs from the URLs (they look like 'tblXXXXXXXXXXXXXX')"
        )
        print(
            "5. Add these IDs to the 'airtable_config.json' file in their respective fields"
        )
        print("6. Restart the application\n")
        print("The app will continue to function without Airtable integration for now.")
        print("=" * 80 + "\n")
